...
def resultsPage(search_stringss):  
    if request.method=="POST":
        ...
        #print(search_string)        
        #return render_template('results.html',search_string=search_string)
        print(request.form.get("myradio1"))
        return redirect(url_for('resultsPage',search_stringss=search_string)) 
    ...
    for i in range(10,1,-1):
        try:
            ...
            #radio1_val = request.form.get('myradio1')
            print(request.form.getlist("myradio1"))
            # if radio1_val =="1":

            #     for j in range(len(AmazonList[0])):
            #         if AmazonList[1][j] > 5000.0:
            #             del AmazonList[:][j]

            #     for j in range(len(FlipkartList[0])):
            #         if FlipkartList[1][j] > 5000.0:
            #             del FlipkartList[:][j]
